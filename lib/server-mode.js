...
function augmentInputWithImages(input, images = []) {
  const stagedPaths = images
    .map((img) => img?.path)
    ...
    switch (msg.type) {
      case "user_input": {
        await handleUserInput(
          augmentInputWithImages(msg.text, msg.images),
          config,
          sessionState,
          tuiAskApproval,
        );
...
async function handleUserInput(input, config, sessionState, tuiAskApproval) {
  ...
  // Track tool args per call so onToolResult can emit files_modified.
    ...
    const result = await executeTarget(
      ...
      sessionState,
      {
        ...
        onToolStart: (tool, reasoning, args, callId) => {
          ...
          if (args) pendingToolArgs.set(key, args);
          send({
            type: "tool_start",
            tool,
            reasoning,
        ...
        onToolResult: (tool, success, summary, elapsedMs, extra = {}) => {
          const key = extra.call_id ?? tool;
          const cachedArgs = pendingToolArgs.get(key);
          ...
          if (success && (tool === "write_file" || tool === "apply_patch")) {
            const path =
              cachedArgs?.path ||
              cachedArgs?.target_file ||
              cachedArgs?.file_path ||
              "unknown";
            send({
              type: "files_modified",
              files: [{ path }],
